...
> **NOTE FOR FOUNDRY V2:** Foundry runs fully autonomously — no human checkpoints in the F0→F6 loop. This file is preserved as a reference for: (1) future hybrid modes where a user wants to gate specific phases, (2) teammates that hit auth gates they genuinely cannot bypass (see "Authentication Gates" section), and (3) borrowing patterns for `/foundry:verify-work` style conversational verification. Everything else in this file describes GSD's interactive workflow and does not apply to Foundry's autonomous runs.
