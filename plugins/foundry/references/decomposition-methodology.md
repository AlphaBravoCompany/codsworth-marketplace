...
> **IMPORTANT: This is a methodology reference, not an active agent.** The concepts (task sizing, vertical slices vs horizontal layers, interface-first ordering, goal-backward methodology, file ownership, anti-patterns) apply directly to Foundry castings. The operational details below use GSD's `PLAN.md` format and `.planning/` directory structure — **these are not used in Foundry**. When you see `PLAN.md` or `.planning/phases/`, mentally substitute `casting` and `foundry-archive/{run}/castings/`. When you see `foundry-tools.cjs` commands, those are GSD-only; Foundry uses MCP tools instead.
>
> **What to extract from this file:**
> - Task anatomy (structure of a single buildable unit)
> - Task sizing and granularity calibration
> - Interface-first task ordering
> - Vertical slices vs horizontal layers
> - File ownership rules for parallel execution
> - Goal-backward methodology (start from what success looks like)
> - Anti-patterns (things decompositions commonly get wrong)
>
> **What to ignore:**
...
Spawned by:
- `/foundry:plan-phase` orchestrator (standard phase planning)
- `/foundry:plan-phase --gaps` orchestrator (gap closure from verification failures)
- `/foundry:plan-phase` in revision mode (updating plans based on checker feedback)
...
The orchestrator provides user decisions in `<user_decisions>` tags from `/foundry:discuss-phase`.
...
For niche domains (3D, games, audio, shaders, ML), suggest `/foundry:research-phase` before plan-phase.
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
INIT=$(node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" init plan-phase "${PHASE}")
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" history-digest
```
...
```bash
cat "$phase_dir"/*-CONTEXT.md 2>/dev/null   # From /foundry:discuss-phase
cat "$phase_dir"/*-RESEARCH.md 2>/dev/null   # From /foundry:research-phase
...
Validate each created PLAN.md using foundry-tools:

```bash
VALID=$(node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node "${CLAUDE_PLUGIN_ROOT}/bin/foundry-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
...
Execute: `/foundry:execute-phase {phase}`
...
- [ ] PLAN file(s) committed to git
- [ ] User knows to run `/foundry:execute-phase {X}` next
