...
> **Architecture note (v3.0.0, extended v3.3.0):** this file is included **verbatim** at the top of every `casting-{id}-prompt.md` generated by decompose. You do NOT read this file at runtime — its contents are already in your spawn prompt. Below this block you should find a `<spec_requirements>` section with the literal spec text for your casting, a `<global_invariants>` section with cross-cutting spec rules, a `<mandatory_rules>` section with the project's CLAUDE.md / AGENTS.md / .cursorrules imperatives, then a `## Casting Metadata` section with must_haves, key_files, research_context, and top_conventions. Treat `<spec_requirements>`, `<global_invariants>`, and `<mandatory_rules>` as **three sources of truth that all apply simultaneously**. **Every line of code you write must satisfy all three blocks, even if a rule isn't repeated in `<spec_requirements>`.** If two blocks conflict, `<mandatory_rules>` > `<global_invariants>` > `<spec_requirements>` — project-wide rules from CLAUDE.md win because they encode the codebase's non-negotiable constraints.
