...
> **Architecture note (v3.0.0):** this file is included **verbatim** at the top of every `casting-{id}-prompt.md` generated by decompose. You do NOT read this file at runtime — its contents are already in your spawn prompt. Below this block you should find a `<spec_requirements>` section with the literal spec text for your casting, then a `## Casting Metadata` section with must_haves, key_files, research_context, and top_conventions. Treat the `<spec_requirements>` block as the source of truth. If anything in your prompt conflicts with it, the `<spec_requirements>` block wins.
