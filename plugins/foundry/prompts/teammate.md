...
> **Architecture note (v3.0.0, extended v3.3.0, reordered v3.5.0 for cache):** this file is included **verbatim** at the top of every `casting-{id}-prompt.md` generated by decompose. You do NOT read this file at runtime — its contents are already in your spawn prompt. Below this block you should find **stable-first, casting-specific-last** ordering (optimizes for prompt-cache reuse across sibling teammates in a wave): `<mandatory_rules>` (project CLAUDE.md / AGENTS.md / .cursorrules imperatives, byte-identical across every casting), then `<global_invariants>` (cross-cutting spec rules, also byte-identical across every casting), then `<spec_requirements>` (this casting's specific acceptance criteria — volatile per-casting content), then `## Casting Metadata` (must_haves, key_files, research_context, top_conventions — also per-casting). Treat `<spec_requirements>`, `<global_invariants>`, and `<mandatory_rules>` as **three sources of truth that all apply simultaneously**. **Every line of code you write must satisfy all three blocks, even if a rule isn't repeated in `<spec_requirements>`.** If two blocks conflict, `<mandatory_rules>` > `<global_invariants>` > `<spec_requirements>` — project-wide rules from CLAUDE.md win because they encode the codebase's non-negotiable constraints. (The physical order in the prompt matches this priority order by design — the strictest-wins hierarchy reads top-down.)
